...
1. 显示[错误的详细信息](cms-pages.md#error-page)。
1. 显示用户身份验证失败的原因。
1. 默认情况下，[合并的静态资源](markup-filter-theme.md) 不会被压缩。
...
> **注意:** 如果使用[Composer](console-commands.md#console-install-composer)管理更新, 然后使用以下内容替换`composer.json`文件中的默认OctoberCMS要求，以便直接从开发分支上下载更新。
