...
October提供的默认变量和Twig扩展名在[标记指南](markup.md)中描述。执行处理程序的整体顺序在[动态布局](cms-layouts.md#dynamic-layouts)文章中有所描述。
...
也可以使用[`this.page` variable](markup-this-page.md)在标记中访问它们。例如：

    <p>这个页面的标题是: {{ this.page.title }}</p>

更多详情请看[`this.page` 标记教程](markup-this-page.md).
...
为了在页面或[布局](layout) 上输出注入的资源，请使用 [{% styles %}](markup-tag-styles.md) 和 [{% scripts %}](markup-tag-scripts.md) 标签。例如：
